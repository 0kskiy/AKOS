/*
 * treasure_condvar.cpp
 * Задача об Острове Сокровищ - Решение на условных переменных (C++)
...
std::condition_variable portfolio_cond;
std::mutex silver_mutex;
std::condition_variable silver_cond;
...
void silver_thread() {
    ...
    std::unique_lock<std::mutex> lock(silver_mutex);
    silver_cond.wait(lock, [] { 
        return groups_finished >= num_groups || stop_flag; 
    });
    
    std::lock_guard<std::mutex> report_lock(report_mutex);
    if (treasure_found) {
        log_message("\n[СИЛЬВЕР] Отлично! Клад найден на участке %d!\n", treasure_sector);
        log_message("[СИЛЬВЕР] Все группы, возвращайтесь на корабль!\n");
    } else {
        log_message("\n[СИЛЬВЕР] Все участки обследованы. Клада нет...\n");
    }
...
void pirate_group_thread(PirateGroup& group) {
    ...
    while (!treasure_found && !stop_flag) {
        ...
        {
            std::unique_lock<std::mutex> lock(portfolio_mutex);
            
            portfolio_cond.wait(lock, [] {
                return current_sector < total_sectors || treasure_found || stop_flag;
            });
            
            if (current_sector >= total_sectors || treasure_found || stop_flag) {
                break;
            }
    ...
    {
        std::lock_guard<std::mutex> lock(silver_mutex);
        groups_finished++;
        silver_cond.notify_one();
    }
}
...
int main(int argc, char* argv[]) {
    ...
    log_message("=== Решение на условных переменных (C++) ===\n\n");
